...
class Home extends Component {
  state = {
    ...
    score: 0,
    time: 60,
    imagesArry: imagesList.filter(
      eachImg => eachImg.category === tabsList[0].tabId,
    ),
    checkingOption: imagesList[0].id,
    currentOpt: tabsList[0].tabId,
    isOut: false,
  }
  ...
  changingOpt = id => {
    this.setState({
      imagesArry: imagesList.filter(eachImg => eachImg.category === id),
      currentOpt: id,
    })
  }

  clickingImage = id => {
    const {checkingOption} = this.state
    if (checkingOption === id) {
      const randomNumber = Math.floor(Math.random() * imagesList.length)
      const updatingImage = imagesList[randomNumber].imageUrl
      const updatingCheecking = imagesList[randomNumber].id
      this.setState(prevState => {
        return {
          checkingOption: updatingCheecking,
          image: updatingImage,
  ...
  render() {
    const {image, score, time, imagesArry, currentOpt, isOut} = this.state
      ...
      <div className="home">
        ...
        {isOut ? (
          <div className="game-part2">
            ...
            <p>Your Score</p>
            <h1>{score}</h1>
            <button onClick={this.playAgain} className="playAgainbutn">
              <img
          ...
          <div className="game-part">
            <img src={image} className="thumnailstyle" alt="match" />
            <ul className="optsUlStyle">
              {tabsList.map(eachopt => (
                <Options
                  changingOpt={this.changingOpt}
                  opt={eachopt}
                  key={eachopt.tabId}
                  light={currentOpt}
                />
            ...
            <ul className="allImagesUl">
              {imagesArry.map(eachAry => (
                <Images
                  key={eachAry.id}
